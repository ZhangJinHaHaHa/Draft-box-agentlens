...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/agent-shenji-local-e2e.XXXXXX")"
...
ATTESTATION_LOG="${TMP_DIR}/attestation.log"
...
cleanup() {
  ...
  if [ -n "${ATTESTATION_PID}" ] && kill -0 "${ATTESTATION_PID}" >/dev/null 2>&1; then
    kill "${ATTESTATION_PID}" >/dev/null 2>&1 || true
    wait "${ATTESTATION_PID}" >/dev/null 2>&1 || true
  fi
...
ATTESTATION_PORT="$(find_free_port 3311)"
...
process.stdout.write(deployment.address);
NODE
)"
...
STAKE_BLOCK_NUMBER="$(
  node - "${STAKE_RESULT_FILE}" <<'NODE'
const fs = require("fs");
const filePath = process.argv[2];
const result = JSON.parse(fs.readFileSync(filePath, "utf8"));
process.stdout.write(String(result.blockNumber));
NODE
)"
...
log_step "Starting local attestation API service"
(
  cd "${SANDBOX_DIR}"
  ...
  npm run --silent run:attestation:api >"${ATTESTATION_LOG}" 2>&1
) &
ATTESTATION_PID=$!
wait_for_http "http://127.0.0.1:${ATTESTATION_PORT}/health"

log_step "Running listener once with writeback enabled"
(
  cd "${SANDBOX_DIR}"
  ...
  AUDIT_ATTESTATION_PROVIDER_TYPE="mock-tee" \
...
const matched =
  objects.find((entry) => entry.type === "audit-processed" && typeof entry.eventKey === "string") ??
  objects.find((entry) => entry.type === "writeback-confirmed" && typeof entry.eventKey === "string");

if (!matched || typeof matched.eventKey !== "string") {
  process.exit(1);
}

process.stdout.write(matched.eventKey);
NODE
)"

if [ -z "${EVENT_KEY}" ]; then
  echo "Failed to resolve eventKey from listener output" >&2
  cat "${LISTENER_OUTPUT_FILE}" >&2
  exit 1
fi

log_step "Verifying persisted local report artifact"
(
  cd "${SANDBOX_DIR}"
  npm run --silent run:report:verify -- --event-key "${EVENT_KEY}" --state-dir "${LISTENER_STATE_DIR}" >"${REPORT_VERIFY_FILE}"
)

log_step "Verifying persisted local evidence artifact"
(
  cd "${SANDBOX_DIR}"
  npm run --silent run:evidence:verify -- --event-key "${EVENT_KEY}" --state-dir "${LISTENER_STATE_DIR}" >"${EVIDENCE_VERIFY_FILE}"
)

log_step "Verifying persisted local attestation artifact"
(
  cd "${SANDBOX_DIR}"
  AUDIT_ATTESTATION_EXPECTED_PROVIDER_TYPE="mock-tee" \
  AUDIT_ATTESTATION_EXPECTED_MEASUREMENT="$(printf 'a%.0s' {1..64})" \
  AUDIT_ATTESTATION_EXPECTED_QUOTE_FORMAT="mock-quote" \
  npm run --silent run:attestation:verify -- --event-key "${EVENT_KEY}" --state-dir "${LISTENER_STATE_DIR}" >"${ATTESTATION_VERIFY_FILE}"
)
...
(async () => {
  ...
  const result = {
    ...
    latestEvidenceRoot: latest.evidenceRoot,
    latestAttestationHash: latest.attestationHash,
    latestEvidenceCID: latest.evidenceCID,
...
LATEST_REPORT_CID="$(
  node - "${READBACK_FILE}" <<'NODE'
const fs = require("fs");
const filePath = process.argv[2];
const result = JSON.parse(fs.readFileSync(filePath, "utf8"));
process.stdout.write(result.latestReportCID ?? "");
NODE
)"
...
const filePath = process.argv[2];
const result = JSON.parse(fs.readFileSync(filePath, "utf8"));
process.stdout.write(result.latestAttestationHash ?? "");
NODE
)"
...
log_step "Local E2E summary"
node - "${DEPLOYMENT_FILE}" "${STAKE_RESULT_FILE}" "${LISTENER_OUTPUT_FILE}" "${READBACK_FILE}" "${FRONTEND_SMOKE_SUMMARY_FILE}" "${REPORT_VERIFY_FILE}" "${EVIDENCE_VERIFY_FILE}" "${ATTESTATION_VERIFY_FILE}" <<'NODE'
const fs = require("fs");
...
const frontendSmoke = JSON.parse(fs.readFileSync(process.argv[6], "utf8"));
const reportVerify = JSON.parse(fs.readFileSync(process.argv[7], "utf8"));
const evidenceVerify = JSON.parse(fs.readFileSync(process.argv[8], "utf8"));
const attestationVerify = JSON.parse(fs.readFileSync(process.argv[9], "utf8"));
...
process.stdout.write(`${JSON.stringify({
  ...
  latestEvidenceRoot: readback.latestEvidenceRoot,
  latestAttestationHash: readback.latestAttestationHash,
  latestEvidenceCID: readback.latestEvidenceCID,
  reportVerifyStatus: reportVerify.status,
  evidenceVerifyStatus: evidenceVerify.status,
  attestationVerifyStatus: attestationVerify.status,
  frontendBaseUrl: frontendSmoke.baseUrl,
  frontendAuditDetailUrl: frontendSmoke.auditDetailUrl
}, null, 2)}\n`);
NODE
