...
require_env EDGE_EXTERNAL_CHAIN_ID
require_env EDGE_EXTERNAL_RPC_PORT
require_env EDGE_EXTERNAL_DEPLOYER_PRIVATE_KEY
require_env EDGE_EXTERNAL_DEPLOYER_ADDRESS
...
if [ ! -f "${NODE_ID_FILE}" ]; then
  echo "Missing ${NODE_ID_FILE}. Remove external chain data and reinitialize the chain." >&2
  exit 1
fi
...
if [ ! -f "${GENESIS_FILE}" ]; then
  polygon-edge genesis \
    --name "${EDGE_EXTERNAL_NETWORK_NAME:-polygon-edge-external}" \
    --consensus ibft \
    --ibft-validator-type bls \
    --chain-id "${EDGE_EXTERNAL_CHAIN_ID}" \
    --block-gas-limit "${EDGE_EXTERNAL_BLOCK_GAS_LIMIT:-20000000}" \
    --premine "${EDGE_EXTERNAL_DEPLOYER_ADDRESS}:1000000000000000000000000" \
    --bootnode "${bootnode}" \
